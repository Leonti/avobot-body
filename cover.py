...
acrylic_plate = translate([0, 0, 1.75])(cylinder(d = acrylic_plate_diameter, h = 3))
...
nut_catch = (
  cylinder(d = 9, h = 3)
  - translate([0, 0, 4])(nuts_n_bolts.cyl_head_bolt.nutcatch_parallel("M3", l=10, clk=0.6))
)
...
def generate_front_left_support(hole_placeholders = False):
  ...
  if hole_placeholders is True:
    return (support_base 
      + translate([-54.5, -86.5, 14])(rotate([90, 90, -30])(cylinder(d = 3.5, h = 30)))
      + bottom_holes
      )
  else:
    return (support_base 
      - translate([-54.5, -86.5, 14])(rotate([90, 90, -30])(mounts.inner_mount_hole))
      - bottom_holes
    )   
...
def generate_front_support(hole_placeholders = False):
  fork_left_hole = translate([0, 7, 0])(rotate([0, -90, 4])(mounts.inner_mount_hole))
  ...
  placeholder_left = translate([-100, -7.25, 14])(rotate([0, -90, 4])(mounts.cylinder(d = 3.5, h = 40)))
  placeholders = (
    placeholder_left
...
full_exploded = (
  ...
  + translate([explode_distance*2, explode_distance*2, explode_distance*2])(back_top_plate)
  + translate([explode_distance*2, explode_distance*2, explode_distance*2])(left_side_plate)
  + translate([explode_distance*2, explode_distance*2, explode_distance*2])(right_side_plate)
  + translate([explode_distance*2, explode_distance*2, explode_distance*2])(front_top_plate)
  + translate([explode_distance*2, explode_distance*2, explode_distance*2])(lidar_cover)
